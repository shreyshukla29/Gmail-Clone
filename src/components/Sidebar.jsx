...
const Sidebar = () => {
    ...
    return (
        <div className='w-[220px] lg:w-[350px]'>
            <div className='p-3'>
                <button onClick={() => dispatch(setOpen(true))} className='flex items-center gap-2 bg-[#C2E7FF] p-4 rounded-2xl hover:shadow-md text-md'>
                    <LuPencil size={"24px"} />
                    Compose
                </button>
            </div>
            <div className='text-gray-500'>
                {
                    sidebarItems.map((item, idx) => {
                        return (
                            <div onClick={() => setSelected(idx)} key={idx} className={`${selected === idx ? 'bg-[#C2E7FF] text-black' : "hover:bg-gray-200 hover:text-black"} flex pl-6 py-1 rounded-r-full items-center gap-4   hover:cursor-pointer`}>
                                {item.icon}
                                <p>{item.text}</p>
                            </div>
                        )
