...
const Inbox = () => {
    ...
    return (
        ...
        <div className='flex flex-col bg-white rounded-xl  h-[90%] max-w-[90%]  min-w-[75%]   mx-5'>
     <div className='flex items-center w-full px-5 py-2  justify-between'>
            ...
            <div className='h-[90vh] overflow-y-auto overflow-x-hidden w-full'>
                <div className='flex items-center gap-1'>
                    {
                        mailType.map((item, index) =>
                            <button
                                key={index}
                                className={`flex items-center gap-4 p-4 ${mailTypeSelected === index ? 'border-b-4 border-b-blue-600 text-blue-600' : 'border-b-4 border-b-transparent'} w-52 hover:bg-gray-100`}
                                onClick={() => {
